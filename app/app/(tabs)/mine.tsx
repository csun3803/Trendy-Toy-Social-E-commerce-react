...
import { View, Text, Image, StyleSheet, TouchableOpacity, ScrollView, Alert, Platform, ImageBackground, Dimensions } from 'react-native';
...
import { getOrderStatusCount } from '@/services/orderService';
...
interface OrderStatusCount {
  unpaid: number;      // 待付款
  unshipped: number;   // 待发货
  shipping: number;    // 待收货（已发货）
...
const Mine = () => {
    ...
    try {
      ...
      if (response.code === 200 && response.data) {
        setOrderStatusCount({
          ...
          unshipped: response.data.unshipped || 0,
          shipping: response.data.shipping || 0,
          completed: response.data.completed || 0,
        });
      }
    } catch (error) {
      console.error('[获取订单数量统计] 失败', error);
    }
  };
  ...
  const fetchUserStats = async () => {
    try {
      console.log('[获取用户统计] 开始获取...');
      const token = await AsyncStorage.getItem('token');
      const userId = await AsyncStorage.getItem('userId');
      console.log('[获取用户统计] Token:', token ? '存在' : '不存在');
      console.log('[获取用户统计] UserId:', userId);
      
      if (!token) {
        console.log('[获取用户统计] 没有token，跳过');
        return;
      }

      const url = `${config.API_BASE_URL}/user/info/current-with-stats`;
      console.log('[获取用户统计] 请求URL:', url);
      ...
      console.log('[获取用户统计] 响应状态:', response.status);
      
      const responseText = await response.text();
      console.log('[获取用户统计] 响应内容:', responseText);
      
      let data;
      try {
        data = JSON.parse(responseText);
      } catch (e) {
        console.error('[获取用户统计] 解析JSON失败:', e);
        return;
      }
      
      console.log('[获取用户统计] 解析后的数据:', data);
      
      if (data.code === 200 && data.data) {
        const backendUser = data.data;
        console.log('[获取用户统计] 后端返回的用户数据:', backendUser);
        console.log('[获取用户统计] 帖子数:', backendUser.postCount);
        console.log('[获取用户统计] 关注数:', backendUser.followingCount);
        console.log('[获取用户统计] 粉丝数:', backendUser.followerCount);
        
        const updatedUserInfo: UserInfo = {
          userId: backendUser.userId || '',
          username: backendUser.username || '未登录',
          avatarUrl: backendUser.avatarUrl || 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=cartoon%20bunny%20avatar%20with%20yellow%20dress%20and%20flower%20in%20hair&image_size=square',
          accountLevel: backendUser.accountLevel || 1,
          postCount: backendUser.postCount !== undefined ? backendUser.postCount : 0,
          followingCount: backendUser.followingCount !== undefined ? backendUser.followingCount : 0,
          followerCount: backendUser.followerCount !== undefined ? backendUser.followerCount : 0,
          totalOrders: backendUser.totalOrders !== undefined ? backendUser.totalOrders : 0,
          couponCount: backendUser.couponCount !== undefined ? backendUser.couponCount : 0,
          cabinetCount: backendUser.cabinetCount !== undefined ? backendUser.cabinetCount : 0,
        };
        
        console.log('[获取用户统计] 更新后的用户信息:', updatedUserInfo);
        setUserInfo(updatedUserInfo);
        await AsyncStorage.setItem('userInfo', JSON.stringify(updatedUserInfo));
        console.log('[获取用户统计] 用户信息已更新');
      } else {
        console.log('[获取用户统计] 请求失败:', data);
      }
    } catch (error) {
      console.error('[获取用户统计] 获取用户统计信息失败', error);
    }
  };
  ...
  const goToPage = (page: string) => {
    if (page === '我的动态') {
      ...
      router.push('/shop?tab=cart' as any);
    } else if (page === '我的关注') {
      router.push('/my-following');
    } else if (page === '我的点赞') {
      router.push('/my-likes' as any);
    } else if (page === '我的收藏') {
      router.push('/my-favorites' as any);
    } else if (page === '我的抽盒记录') {
      router.push('/my-draw-records' as any);
    }
  };
    ...
    <View style={styles.minePage}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.userInfoContainer}>
          <Image 
            source={require('../../assets/images/mineBackground.png')} 
            ...
            <View style={styles.userInfo}>
              <TouchableOpacity onPress={handleAvatarUpload} style={styles.avatarContainer}>
                {(() => {
                  ...
                  imageUri = imageUri.replace(/http:\/\/localhost:8080/g, BASE_URL);
                  imageUri = imageUri.replace(/http:\/\/10\.0\.2\.2:8080/g, BASE_URL);
                  return (
                    <Image 
                      source={{ uri: imageUri }} 
                      style={styles.avatar} 
        ...
        <View style={styles.cardsContainer}>
          <TouchableOpacity style={styles.cardItem} onPress={() => goToPage('我的盒柜')}>
            <Text style={styles.cardNumber}>{userInfo?.cabinetCount || 0}</Text>
            <Text style={styles.cardLabel}>我的盒柜</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.cardItem} onPress={() => goToPage('我的收藏')}>
            <Text style={styles.cardNumber}>{userInfo?.postCount || 0}</Text>
            <Text style={styles.cardLabel}>我的收藏</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.cardItem} onPress={() => goToPage('我的关注')}>
            <Text style={styles.cardNumber}>{userInfo?.followingCount || 0}</Text>
            <Text style={styles.cardLabel}>我的关注</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.cardItem} onPress={() => goToPage('我的券包')}>
            <View style={styles.couponWrapper}>
              <Text style={styles.cardNumber}>{userInfo?.couponCount || 0}</Text>
              {(userInfo?.couponCount || 0) > 0 && (
                <View style={styles.couponBadge}>
                  <Text style={styles.couponBadgeText}>未使用</Text>
                </View>
              )}
            </View>
            <Text style={styles.cardLabel}>我的券包</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.sectionContainer}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>我的订单</Text>
              <TouchableOpacity onPress={() => goToOrders('all')} style={styles.viewAllButton}>
          ...
          <View style={styles.orderStatusContainer}>
            <TouchableOpacity style={styles.orderStatusItem} onPress={() => goToOrders('unpaid')}>
              <View style={styles.orderIconWrapper}>
                <Svg width={36} height={36} viewBox="0 0 1037 1024" fill="none">
                  <Path
                ...
                {orderStatusCount.unpaid > 0 && (
                  <View style={styles.orderBadge}>
                    <Text style={styles.orderBadgeText}>{orderStatusCount.unpaid > 99 ? '99+' : orderStatusCount.unpaid}</Text>
                  </View>
                )}
              </View>
            ...
            <TouchableOpacity style={styles.orderStatusItem} onPress={() => goToOrders('unshipped')}>
              <View style={styles.orderIconWrapper}>
                ...
                {orderStatusCount.unshipped > 0 && (
                  <View style={styles.orderBadge}>
                    <Text style={styles.orderBadgeText}>{orderStatusCount.unshipped > 99 ? '99+' : orderStatusCount.unshipped}</Text>
                  </View>
                )}
              </View>
            ...
            <TouchableOpacity style={styles.orderStatusItem} onPress={() => goToOrders('shipping')}>
              <View style={styles.orderIconWrapper}>
                ...
                {orderStatusCount.shipping > 0 && (
                  <View style={styles.orderBadge}>
                    <Text style={styles.orderBadgeText}>{orderStatusCount.shipping > 99 ? '99+' : orderStatusCount.shipping}</Text>
                  </View>
                )}
              </View>
            ...
            <TouchableOpacity style={styles.orderStatusItem} onPress={() => goToOrders('completed')}>
              <View style={styles.orderIconWrapper}>
                ...
                {orderStatusCount.completed > 0 && (
                  <View style={styles.orderBadge}>
                    <Text style={styles.orderBadgeText}>{orderStatusCount.completed > 99 ? '99+' : orderStatusCount.completed}</Text>
                  </View>
                )}
              </View>
          ...
          <View style={styles.functionList}>
            <TouchableOpacity style={styles.functionItem} onPress={() => goToPage('我的购物车')}>
              <Text style={styles.functionText}>我的购物车</Text>
            ...
            <TouchableOpacity style={styles.functionItem} onPress={() => goToPage('我的点赞')}>
            ...
            <TouchableOpacity style={styles.functionItem} onPress={() => goToPage('我的抽盒记录')}>
              <Text style={styles.functionText}>我的抽盒记录</Text>
              <Svg width={12} height={12} viewBox="0 0 24 24">
                <Path
                  d="M8.59 16.59L13.17 12 8.59 7.41 10 6l6 6-6 6-1.41-1.41z"
                  fill="#CCC"
