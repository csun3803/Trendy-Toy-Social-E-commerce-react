...
export default function CommentNotificationsScreen() {
  ...
  const fetchNotifications = async () => {
    setLoading(true);
    try {
      const response = await api.get('/interaction/notifications/comments');
  ...
  const renderItem = (item: CommentNotification) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => router.push(`/activity/${item.activity_id}` as any)}
      >
        ...
        <View style={styles.content}>
          <View style={styles.headerRow}>
            ...
            <Text style={styles.time}>{formatTime(item.commented_at)}</Text>
          </View>
          <Text style={styles.actionText}>评论了你的动态</Text>
          {item.activity_title ? (
            <Text style={styles.activityTitle} numberOfLines={1}>《{item.activity_title}》</Text>
          ) : null}
          <Text style={styles.commentContent} numberOfLines={2}>{item.content}</Text>
        </View>
        <Svg width={20} height={20} viewBox="0 0 1024 1024" fill="none">
          <Path
            d="M810.666667 149.333333a128 128 0 0 1 128 128v362.666667a128 128 0 0 1-128 128h-184.896l-92.48 138.709333a25.6 25.6 0 0 1-5.056 5.610667l-2.026667 1.493333a25.6 25.6 0 0 1-33.834667-4.864l-1.664-2.24L398.229333 768H213.333333a128 128 0 0 1-128-128V277.333333a128 128 0 0 1 128-128h597.333334z"
            fill="#FFC500"
          />
        </Svg>
      </TouchableOpacity>
  ...
  return (
    <View style={styles.page}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>评论互动</Text>
        <View style={{ width: 30 }} />
      </View>
      <ScrollView
      ...
      >
        {notifications.length > 0 ? (
          ...
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>暂无评论通知</Text>
          </View>
        )}
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  activityTitle: { fontSize: 12, color: '#8069E1', marginTop: 2 },
  commentContent: { fontSize: 13, color: '#333', marginTop: 4, lineHeight: 18 },
});
